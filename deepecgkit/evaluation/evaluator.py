...
class ECGEvaluator:
    ...
    def _process_numpy_input(
        ...
        if len(test_data) == TUPLE_LENGTH_3:
            targets = test_data[1]
            self.y_scores = test_data[2]
        elif len(test_data) == TUPLE_LENGTH_2:
            targets = test_data[1]
        else:
            targets = test_data
    ...
    def _get_predictions(
        ...
        elif isinstance(test_data, tuple) and len(test_data) == TUPLE_LENGTH_2:
    ...
    def _process_dataloader(
        ...
        with torch.no_grad():
            for batch in test_data:
                if isinstance(batch, (list, tuple)) and len(batch) == TUPLE_LENGTH_2:
                    x, y = batch
                else:
                    x, y = batch, None
